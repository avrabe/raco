...
impl WorkflowInstance {
    ...
    pub fn new(definition: WorkflowDefinition) -> Result<Self> {
        let step_ids: HashSet<_> = definition.steps.iter()
            .map(|step| step.id())
            .collect();
        ...
        for (from, to) in &definition.dependencies {
            if !step_ids.contains(from) {
                return Err(anyhow::anyhow!("Invalid dependency: step {} not found", from));
            }
...
mod tests {
    ...
    async fn test_workflow_lifecycle() {
        ...
        assert_eq!(instance.unwrap().lock().await.status(), WorkflowStatus::Pending);
